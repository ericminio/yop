...
const open = (spec) => {
    ...
    let port = isUrl ? parseInt(spec.substring(spec.indexOf(PREFIX) + PREFIX.length)) : 0;
    return new Promise(async (resolve, reject) => {
        try {
            const dom = await openWithJsdom(isUrl)(target, config);
            page.window = dom.window;
            page.window.fetch = fetch(port);
...
const set = (prompt) => {
    ...
    if (label.htmlFor.length === 0) {
        throw new Error(`label with text '${prompt}' is missing htmlFor attribute`);
    }
