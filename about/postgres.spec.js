...
describe('Postgres', () => {
    ...
    it('can execute one select', async () => {
        ...
        const rows = await database.executeSync("select 'Joe' as name");
    ...
    it('can execute one select with one parameter', async () => {
        const database = new Postgres();
        const rows = await database.executeSync('select $1 as name', ['Jack']);
        expect(rows.length).to.equal(1);
    ...
    it('can execute several statements and return the last results', async () => {
        const database = new Postgres();
        const rows = await database.executeSync([
